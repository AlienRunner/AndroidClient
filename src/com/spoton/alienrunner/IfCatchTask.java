...
public class IfCatchTask extends AsyncTask<ArrayList<User>, Void, Boolean> {
	...
	protected Boolean doInBackground(ArrayList<User>... params) {
		...
		while (it.hasNext()) {
			...
			distance = calcDistance((float) myUser.getxCoord(),
					(float) myUser.getyCoord(), (float) opponent.getxCoord(),
					(float) opponent.getyCoord());
			if (distance < 10) {
				return true;
			}
	...
	public static int calcDistance(float latA, float longA, float latB,
			float longB) {
		...
		return new Double(
				(Math.toDegrees(Math.acos(theDistance))) * 69.09 * 1.6093)
				.intValue();
